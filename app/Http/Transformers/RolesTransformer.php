...
class RolesTransformer
{
    private static function transformRole($role)
    {
        ...
        $permissions_array['available_actions'] = [
            'update' => (Auth::user()->can('Update Role') && ($role->deleted_at=='') && $role->name != 'Super Admin')  ? true : false,
            'delete' =>(Auth::user()->can('Delete Role') && ($role->deleted_at=='') && $role->name != 'Super Admin') ? true : false,
            'restore' => (Auth::user()->can('Create Role') && ($role->deleted_at!='') && $role->name != 'Super Admin') ? true : false,
        ];
